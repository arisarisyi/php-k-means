...
<div id="content">
  ...
  $result = mysqli_query($connect, "SELECT Id, Nama, Nahwu, Shorof,Hadits, Fiqih, Akhlaq FROM santri ");
  $total = mysqli_num_rows($result);
  $pages = ceil($total/$halaman);            
  $query = mysqli_query($connect, "SELECT Id, Nama, Nahwu, Shorof,Hadits, Fiqih, Akhlaq FROM santri LIMIT $mulai, $halaman ")or die(mysql_error);
  $no =$mulai+1;
  ...
  while ($data = mysqli_fetch_assoc($query)) {
    ?>
				<tr>
					...
					<td><?php echo $data['Nahwu']; ?></td>
					<td><?php echo $data['Shorof']; ?></td>
					<td><?php echo $data['Hadits']; ?></td>
					<td><?php echo $data['Fiqih']; ?></td>
					<td><?php echo $data['Akhlaq']; ?></td>
